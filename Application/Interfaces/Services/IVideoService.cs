...
{
    Task<IEnumerable<VideoDto>> GetVideosAsync(int page = 1, int pageSize = 20, Guid? currentUserId = null);
    Task<IEnumerable<VideoDto>> GetVideosByUserIdAsync(Guid uploaderUserId, Guid? currentUserId = null);
    Task<VideoDto?> GetVideoByIdAsync(Guid videoId, Guid? currentUserId = null);
